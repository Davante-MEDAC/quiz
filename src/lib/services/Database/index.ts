...
import type { IDatabase, DbUser, DbAccount, DbSession, DbVerificationToken } from './IDatabase.js';
...
export class Database implements IDatabase {
	...
	private mapUser(row: Row): DbUser {
		return {
			...
			email_verified: row['email_verified'] ? new Date(row['email_verified'] as string) : null,
	...
	async createUser(user: Omit<DbUser, 'created_at' | 'updated_at'>): Promise<DbUser> {
	...
	async updateUser(user: Partial<DbUser> & Pick<DbUser, 'id'>): Promise<DbUser> {
		const fields: string[] = [];
	...
	async linkAccount(account: Omit<DbAccount, 'created_at' | 'updated_at'>): Promise<DbAccount> {
		await this.client.execute({
	...
	async getAccount(providerAccountId: string, provider: string): Promise<DbAccount | null> {
		const result = await this.client.execute({
	...
	async createSession(session: Omit<DbSession, 'created_at' | 'updated_at'>): Promise<DbSession> {
		await this.client.execute({
			sql: `INSERT INTO authjs_sessions (id, user_id, session_token, expires)
            VALUES (?, ?, ?, ?)`,
			args: [session.id, session.user_id, session.session_token, session.expires.toISOString()]
		});
		const result = await this.client.execute({
	...
	async createVerificationToken(token: DbVerificationToken): Promise<DbVerificationToken> {
		await this.client.execute({
