<script lang="ts">
	...
	function getOptionClass(index: number): string {
		const base =
			'bg-transparent border-2 border-border rounded-xl px-5 py-4 text-left cursor-pointer transition-all duration-200 text-base text-white';

		if (!quizState.showFeedback) {
			return quizState.selectedAnswer === index
				? `${base} !border-primary bg-primary/20`
				: `${base} hover:border-primary hover:bg-primary/10`;
		}

		// Show feedback
		if (index === question.correctAnswer) {
			return `${base} !border-primary bg-primary/30`;
		}
		if (index === quizState.selectedAnswer && index !== question.correctAnswer) {
			return `${base} !border-danger bg-danger/30`;
		}
		return base;
	}
</script>
...
<div class="bg-surface border-border mb-5 animate-[fadeIn_0.3s_ease-in] rounded-2xl border-2 p-6">
	<h2 class="mb-5 text-xl leading-tight font-semibold">{question.question}</h2>
